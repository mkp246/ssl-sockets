...
function info(message)
  vlc.msg.info(prefix..message)
end
function err(message)
  vlc.msg.err(prefix..message)
end
function warn(message)
  vlc.msg.warn(prefix..message)
end
function dbg(message)
  vlc.msg.dbg(prefix..message)
end
...
function showMainDialog()
  ...
  dialog:add_button("Search by name", searchByName, 4, 2, 1, 1)
...
function searchByName()
  ...
  local fd = vlc.net.connect_tcp("subscene.com", 443)
  if not fd then
  ...
  -- local header = {
  --   "GET ".."/subtitles/release?q=mr%20robot%20s03e09".." HTTP/1.1",
  --   "Host: ".."subscene.com",
  ...
  -- dbg("http request is:")
  -- dbg(request)
  -- dbg("http request printed")
  t = {}
  for i=1,26 do
    t[i] = string.char(math.random(65, 90))
  end
  local rand = table.concat(t)
  local hello = {0x16, 0x03,0x01, 0x00, 0x63, 0x01, 0x00,0x00,0xbf, 0x03,0x03, 0xb1,0x65,0x30,0xb7} --+ add here 28 byte rand
              --rectype versio rec len hsTyp len      tls v          
  local hello2 = {0x00, 0x00,0x06, 0xc0,0x2b, 0xc0,0x2f, 0xc0,0x2c, 0x01, 0x00 , 0x00}
          --sessionidL csL ciphers..............compressMeth0 ext len
  local helloMsgFinal = ""
  for i,v in pairs(hello) do
    helloMsgFinal = helloMsgFinal..string.char(v)
  end
  helloMsgFinal = helloMsgFinal..rand
  for i,v in pairs(hello2) do
    helloMsgFinal = helloMsgFinal..string.char(v)
  end
  dbg(string.len(helloMsgFinal))
  dbg(helloMsgFinal)
  vlc.net.send(fd, helloMsgFinal)
  vlc.net.poll(pollfds)

  local head, body, contentType, contentLength, status, transferEncoding, chunked

  local response = vlc.net.recv(fd, 2048)
  dbg("http response is:")
  local lenth = 0
  ...
  do
    ...
    vlc.net.poll(pollfds)
    response = vlc.net.recv(fd, 2048)
  end
  dbg("http response printed length: "..length)
end
